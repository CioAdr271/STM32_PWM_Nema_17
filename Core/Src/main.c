...
//Pentru frecventa am setat limite normale in functionearea fara microstepping
...
void accelerate(int target_speed) {
	...
	while (current_frequency < target_speed) {

		current_frequency += ACCELERATION_STEP;

		if (current_frequency > target_speed) {
			current_frequency = target_speed;
		}
